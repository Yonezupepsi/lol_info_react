...
export const Info =  () => {
    ...
    const apirequest = async () => { // api 연결
        try {
            const response = await axios.get(`https://ddragon.leagueoflegends.com/cdn/13.23.1/data/ko_KR/champion.json`);// 1번째 연결
            if (response.data.data) {// 데이터가 있으면 Id, Name을 저장
    ...
    return (
        <>
        <styledImg>
            {/* <div onClick={apirequest}>
        champion</div> */}
            <div onClick={apirequest}>champion</div>
            {championName}<br></br>
            <img src={`https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${championId}_0.jpg`} alt={championName} /><br></br>
            {championInfo}
        </styledImg>
